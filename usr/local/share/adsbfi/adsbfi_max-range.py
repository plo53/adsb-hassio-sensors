...
# Max range from geodesic aircraft distance (adsb.fi)
...
if __name__ == "__main__":
    ...
    url =  '/run/adsbfi-feed/aircraft.json'
    ...
    with open(url) as response:
        ...
        if (sorted_dist and ulat != 0 and ulon != 0):
            sensor_mr = {"state": int(sorted_dist[0][1]),
                         "attributes": {"friendly_name": "Adsb.fi Max Range",
                         "icon": "mdi:map-marker-distance",
                         "state_class": "measurement",
                         "unit_of_measurement": "nm",
                         "unique_id": "adsbfi_max_range"} }
        else:
            sensor_mr = {"state": "0",
                         "attributes": {"friendly_name": "Adsb.fi Max Range",
                         "icon": "mdi:map-marker-distance",
